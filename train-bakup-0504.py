...
def main(args):
    ...
    with torch.no_grad():
        z_train_raw = encode_mu(protein_vae, x_protein)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # noise endpoint in standardized latent space
        else:
            z_base = None
            z0 = torch.randn_like(z1)

        if args.t_bias_to_one:
            u = torch.rand(z1.size(0), device=device)
            t = 1.0 - u ** 2
        else:
            t = torch.rand(z1.size(0), device=device)
        ...
        loss = (   fm_loss  + args.endpoint_weight * endpoint_loss  + args.prior_weight * prior_loss)
        ...
        # ---------- validation on full val subgraph ----------
...
if __name__ == "__main__":
    ...
    parser.add_argument("--spatial_weight", type=float, default=0.2)
    ...
    parser.add_argument("--prior_noise_scale", type=float, default=0.1)
